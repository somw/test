...
def order_role():
    ...
    order_dick = {}
    for i in range(3):
        order = int(input('你想将%s放在第几个上场？（输入数字1~3）' % players[i]))
        order_dick[order] = players[i]

    players = []
    for i in range(1,4):
        players.append(order_dick[i])
    print('我方角色的出场顺序是 %s、%s、%s' %(players[0],players[1],players[2]))
